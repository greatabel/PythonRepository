function plot_results(agent,nsteps,fmode,outImages)
    ...
    if (fmode==false) || (N_IT==nsteps) || ((fmode==true) && (rem(N_IT , CONTROL_DATA.fmode_display_every)==0))
        ...
        col{1}='r-';                   %set up colours that will represent different cell types red for red squirrels, blue for grey squirrels
        col{2}='b-';

        tot_food=IT_STATS.tfood;       %total food remaining
        n=nr(N_IT+1)+ng(N_IT+1);             %current agent number
        f2=figure(2);
        ...
        axis_lim = max(ng, nr);
        subplot(3,1,1),axis([0 nsteps 0 1.1*max(axis_lim)]);
        subplot(3,1,2),cla
        subplot(3,1,2),plot((1:N_IT+1),ng(1:N_IT+1),col{2});
        subplot(3,1,2),axis([0 nsteps 0 1.1*max(axis_lim)]);
        subplot(3,1,3),cla
        subplot(3,1,3),plot((1:N_IT+1),tot_food(1:N_IT+1),'m-');
        subplot(3,1,3),axis([0 nsteps 0 tot_food(1)]);
        subplot(3,1,1),title('No. live red squirrels');
        subplot(3,1,2),title('No. live grey squirrels');
        ...
        for cn=1:length(agent)                          %cycle through each agent in turn
            if typ(cn)>0                                %only plot live agents
                ...
                if isa(agent{cn},'red_squirrel')              %choose plot colour depending on agent type
